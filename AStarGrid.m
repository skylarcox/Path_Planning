...
%     % Update input_map
    ...
    %neighborNodes = [current + 1, current + nrows, current - nrows, current - 1];
    
    [xRow, yCol] = ind2sub(size(g), neighborNodes);
    ...
    for iNode = 1:numNeighbors
        ...
        [iNeiX, iNeiY] = ind2sub(size(g), currNei);
        
        if ~(map(currNei) == 3 || map(currNei) == 2 || map(currNei) == 5)
            map(currNei) = 4;
            g(currNei) = g(current) + 1; %tentDistVal; %min(g(currNei),tentDistVal);
            
            % Update f
            f(currNei) = g(currNei) + H(currNei);
            
            if ~(keepTrack(currNei))
                parent(currNei) = current;
                keepTrack(currNei) = true;
            end   
        end
    end
